...
if numero_cd:
    ...
    if not registros_filtrados.empty:
        ...
        st.write(
            registros_filtrados[["Nº", "AUTOR", "NOMBRE CD"]].to_html(index=False, classes="table table-striped"),
            unsafe_allow_html=True,
        )
...
if st.checkbox("Mostrar todos los registros actuales"):
    ...
    st.write(
        datos[["Nº", "AUTOR", "NOMBRE CD"]].to_html(index=False, classes="table table-striped"),
        unsafe_allow_html=True,
    )